Module.register('MMM-HVV', {
  ...
  start: function () {
    Log.log(
      'HVV frontend started. ' +
        `Departures from station ${this.config.station} to destination ${this.config.destination}.`
    );
  ...
  fetchHVV: function () {
    var destination = this.config.destination
      ? `&direction=${this.config.destination}`
      : ``;
    ...
    fetch(endpoint)
      ...
      .then((data) => {
        for (var key in data) {
          ...
          var icon = this.config.showIcons
      ...
      .catch((error) => {
        Log.error(
          this.name +
            ': Something went wrong. Please check modules config. ' +
            error
        );
      });
  },
